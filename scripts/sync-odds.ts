...
async function main() {
  ...
  for (const fixture of fixtures) {
    ...
    matched += 1;
    if (eventMatch.strategy === "date+teams") {
      dateMatched += 1;
    } else {
      pairMatched += 1;
    }
  ...
  console.log(
    `Fetched ${events.length} ${sportKey} odds events. Matched ${matched}/${fixtures.length} fixtures (${dateMatched} date+teams, ${pairMatched} teams-only) and updated odds for ${updated}.`
  );

  if (unmatched.length > 0) {
    console.log("First unmatched seeded fixtures:");
    for (const match of unmatched.slice(0, 8)) {
      console.log(`- ${match.kickoffAt.toISOString().slice(0, 10)}: ${match.homeTeam.name} vs ${match.awayTeam.name}`);
    }
  }
}
...
function findEvent(events: OddsEvent[], match: MatchWithTeams): EventMatch | null {
  ...
  if (sameDay) {
    return { event: sameDay, strategy: "date+teams" };
  }
  ...
  if (samePair.length === 1) {
    return { event: samePair[0], strategy: "teams" };
  }
...
function impliedProbabilities(event: OddsEvent, match: MatchWithTeams) {
  ...
  for (const bookmaker of event.bookmakers) {
    ...
    if (home && away) {
      moneylines.push({ home, draw, away, updatedAt: bookmaker.last_update });
    }
  }
